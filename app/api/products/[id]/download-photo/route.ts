...
function buildLabelSvg(label: string, imgWidth: number): { svg: Buffer; boxWidth: number; boxHeight: number; margin: number } {
  ...
  const svg = `<svg width="${boxWidth}" height="${boxHeight}" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="${padX}" y="50%" dominant-baseline="middle" font-family="sans-serif" font-size="${fontSize}" fill="#ffffff">${escapeXml(label)}</text>
  </svg>`;
...
export async function GET(_req: NextRequest, ctx: RouteContext<"/api/products/[id]/download-photo">) {
  ...
  const { svg, boxWidth, boxHeight, margin } = buildLabelSvg(label, imgWidth);
