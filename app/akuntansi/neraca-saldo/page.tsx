...
export default async function NeracaSaldoPage({ searchParams }: PageProps<"/akuntansi/neraca-saldo">) {
  ...
  return (
    <>
      ...
      <Panel id="report-doc">
        <PanelHead title={`Neraca Saldo (Trial Balance) — sampai akhir ${MONTH_NAMES[month - 1]} ${year}`} />
      <TableScroll>
        <table className="w-full border-collapse">
          ...
          <tbody>
            {balances.map((b) => (
              <tr key={b.code} className="hover:bg-[#fbfaf5]">
                <td className="border-b border-line px-5 py-3 font-mono text-[0.78rem]">{b.code}</td>
                <td className="border-b border-line px-5 py-3 font-medium">{b.name}</td>
                <td className="border-b border-line px-5 py-3 font-mono text-[0.72rem] text-muted">{b.kelompok}</td>
                <td className="border-b border-line px-5 py-3 text-right font-mono text-[0.78rem]">
        ...
        <div className={`px-5 py-3 font-mono text-[0.75rem] ${balanced ? "text-moss-deep" : "text-danger"}`}>
