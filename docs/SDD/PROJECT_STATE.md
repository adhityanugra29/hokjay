...
- **TASK-026** (+ same-day follow-up) — Invoice "Catatan" free-text field, shown on Invoice/Bukti Transfer AND Surat Jalan PDFs/previews. Beranda's "Perlu Ditindak" widget reframed as "Perlu Dikirim" — sorted/labeled by `tanggalKirim` shipping urgency (new `shippingUrgency()` helper, `lib/dashboard.ts`, label now always includes the actual date too) instead of days-unpaid, applied to all 4 admin/sales × desktop/mobile surfaces. Tanggal Pengiriman no longer defaults to H+3. "Stok tipis" row removed from the reframed desktop widget. Not yet click-tested live.
