...
TASK-027 planned, not yet built: filters on the two "table" surfaces under Pelanggan — `/pelanggan`'s "Semua Pelanggan" list gets a SearchInput (nama/kode) + Kota dropdown, `/pelanggan/[id]`'s "Semua Invoice Pelanggan Ini" gets a status pill filter (Draft/Belum Bayar/Lunas). Confirmed with the user via AskUserQuestion before coding — plan locked, ready to build next.

TASK-026 (Invoice Catatan field + PDF, Beranda "Perlu Dikirim" reframe across all 4 surfaces, Tanggal Pengiriman no longer defaults, + 3 same-day follow-ups) is DONE and screenshot-confirmed live/working by the user. TASK-002's Purchasing/Payroll/Admin sweep (see IN PROGRESS below) is still the last known open thread from the Foundry rework if the user wants to resume it.
