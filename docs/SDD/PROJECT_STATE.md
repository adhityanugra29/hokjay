...
TASK-026 (Invoice Catatan field + PDF, Beranda "Perlu Dikirim" reframe across all 4 surfaces, Tanggal Pengiriman no longer defaults, + 4 same-day follow-ups including a real "Tandai Sudah Kirim" trigger — see LAST COMPLETED) is DONE. Build/lint/typecheck clean; the earlier parts of this task chain were screenshot-confirmed live/working by the user, follow-up 4 (Tandai Sudah Kirim) has not yet been click-tested live. TASK-002's Purchasing/Payroll/Admin sweep (see IN PROGRESS below) is still the last known open thread from the Foundry rework if the user wants to resume it.
